...
export default function Dashboard() {
    ...
    const devices = [
        { imei: "123456", interval: 20, geoid: "Ranchi", status: "active", battery: 85, signal: 92, lastSeen: "2 min ago", alerts: 0 },
        { imei: "789012", interval: 95, geoid: "Delhi", status: "idle", battery: 45, signal: 78, lastSeen: "15 min ago", alerts: 2 },
        { imei: "345678", interval: 30, geoid: "Mumbai", status: "active", battery: 92, signal: 95, lastSeen: "Just now", alerts: 0 },
        { imei: "901234", interval: 120, geoid: "Bangalore", status: "offline", battery: 12, signal: 0, lastSeen: "2 hours ago", alerts: 1 },
    ]

    const recentAlerts = [
        { id: 1, device: "789012", message: "Low battery", time: "5 min ago", severity: "warning" },
        { id: 2, device: "901234", message: "Device offline", time: "2 hours ago", severity: "error" },
        { id: 3, device: "123456", message: "Movement detected", time: "10 min ago", severity: "info" },
    ]
                ...
                <CardContent className="p-6">
                    <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            {selectedDevice !== "all" && (
                                <Button variant="outline" onClick={() => setSelectedDevice("all")} className="gap-2">
                                    Clear
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="p-4 bg-green-50 dark:bg-green-950/50 rounded-lg">
                                        ...
                                        <p className="text-2xl font-bold text-green-700 dark:text-green-400">156</p>
                                    </div>
                                    <div className="p-4 bg-blue-50 dark:bg-blue-950/50 rounded-lg">
                                        <p className="text-sm text-muted-foreground">Avg Distance</p>
                                        <p className="text-2xl font-bold text-blue-700 dark:text-blue-400">45 km</p>
                                    </div>
                                    <div className="p-4 bg-purple-50 dark:bg-purple-950/50 rounded-lg">
                                        <p className="text-sm text-muted-foreground">Total Hours</p>
                                        <p className="text-2xl font-bold text-purple-700 dark:text-purple-400">328h</p>
                                    </div>
                                </div>
                                <div className="h-[100px] bg-gradient-to-r from-green-100 to-blue-100 dark:from-green-900/20 dark:to-blue-900/20 rounded-lg flex items-center justify-center">
                                    ...
                                    <span className="ml-2 text-muted-foreground">Trip trend chart</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-none shadow-lg bg-gradient-to-br from-gray-900 to-gray-800 text-white">
                        ...
                        <CardContent className="p-6 space-y-4">
                            <div className="space-y-3">
                                {[
                                    { label: 'Total Distance', value: '245 km', icon: Navigation, color: 'text-blue-400' },
                                    { label: 'Active Now', value: '4 devices', icon: Activity, color: 'text-green-400' },
                                    { label: 'Avg Interval', value: '42s', icon: Clock, color: 'text-purple-400' },
                                    { label: 'Data Usage', value: '2.4 GB', icon: Signal, color: 'text-orange-400' },
                                ].map((item, index) => {
                            ...
                            <div className="pt-4 border-t border-gray-700">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <Badge variant="outline" className="border-green-500 text-green-400">98%</Badge>
                                </div>
                                <Progress value={98} className="h-2 bg-gray-700" indicatorClassName="bg-gradient-to-r from-green-400 to-green-500" />
                            </div>
                        </CardContent>
                    </Card>
                            ...
                            {recentAlerts.map((alert) => (
                                <div key={alert.id} className="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-900 rounded-lg">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-muted-foreground">Device: {alert.device} • {alert.time}</p>
                                    </div>
                                </div>
                            ))}
                        </CardContent>
            ...
            <Card className="border-none shadow-lg">
                ...
                <CardContent>
                    <Table>
                        ...
                        <TableBody>
                            {devices.map((device) => (
                                <TableRow key={device.imei} className="hover:bg-gray-50 dark:hover:bg-gray-900/50">
                                    <TableCell>
                                        <div className="flex items-center gap-2">
                                            <div className={`w-2.5 h-2.5 rounded-full ${getStatusColor(device.status)} animate-pulse`} />
                                            <span className="capitalize">{device.status}</span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="font-mono font-medium">{device.imei}</TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-2">
                                            <Battery className={`h-4 w-4 ${device.battery > 70 ? 'text-green-500' :
                                                    device.battery > 30 ? 'text-yellow-500' : 'text-red-500'
                                                }`} />
